import Link from 'next/link';

const h1Styles = 'mb-3 text-3xl font-semibold';
const mainStyles = `flex flex-col items-center justify-between p-12 before:absolute before:h-[100vh] before:w-[480px] before:-translate-x-1/2 before:rounded-full before:bg-gradient-radial before:from-white before:to-transparent before:blur-2xl before:content-[''] after:absolute after:-z-20 after:h-[180px] after:w-[240px] after:translate-x-1/3 after:bg-gradient-conic after:from-sky-200 after:via-blue-200 after:blur-2xl after:content-[''] before:dark:bg-gradient-to-br before:dark:from-transparent before:dark:to-blue-700 before:dark:opacity-10 after:dark:from-sky-900 after:dark:via-[#0141ff] after:dark:opacity-40 before:lg:h-[360px] z-[-1] lg:gap-12`;
const appsSection =
    'mb-32 grid text-center lg:max-w-5xl lg:w-full lg:mb-0 lg:grid-cols-4 lg:text-left';
const appTitle = 'mb-3 text-2xl font-semibold';
const appDescription = 'm-0 max-w-[30ch] text-sm opacity-50';

function Home() {
    return (
        <main className={mainStyles}>
            <header>
                <h1 className={h1Styles}>Privacy Policies Manager</h1>
            </header>

            <div className={appsSection}>
                <Link href={'/esnap'}>
                    <h2 className={appTitle}>
                        Esnap{' '}
                        <span className='inline-block transition-transform group-hover:translate-x-1 motion-reduce:transform-none'>
                            -&gt;
                        </span>
                    </h2>
                    <p className={appDescription}>
                        A convenient app for managing your wardrobe.
                    </p>
                </Link>
            </div>
        </main>
    );
}

export default Home;
